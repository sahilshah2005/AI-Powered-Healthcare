...
class PredictionForm(forms.Form):
    age = forms.IntegerField(min_value=1, max_value=120, label='Age (in years)')
    ...
    trestbps = forms.FloatField(min_value=50, max_value=300, label='Resting Blood Pressure (mm Hg)')
    chol = forms.FloatField(min_value=50, max_value=600, label='Serum Cholestoral (mg/dl)')
    ...
    thalach = forms.FloatField(min_value=40, max_value=250, label='Maximum Heart Rate')
    exang = forms.TypedChoiceField(choices=[(1, 'Yes'), (0, 'No')], coerce=int, label='Exercise Induced Angina')
    oldpeak = forms.FloatField(min_value=0.0, max_value=10.0, label='ST Depression (Oldpeak)')
